...
var { getBuffer } = require(__path + '/lib/fetcher.js')
...
const listkey = ["Rafasurya", "Rafasurya"];
...
var len = 15
        ...
        var randomTextNumber = random+randomlagi+'Rafa'+'Surya';

router.get('/photooxy/party', async (req, res) => {/*succes*/
    var teks = req.query.teks
    res.header("Content-Type",'application/json');
    if(teks==undefined||teks==""||teks==" "){ return res.send(JSON.stringify({creator: "M Hadi Firmansya",status: false,msg: "masukkan parameter teks"}, null, 4));}
    config = {
	method: 'post',
	...
	data: `text_2=${teks}&login=OK`
}
	try{await axios(config).then((web)=>{dt = web.data.split(`<div class="col-xs-4">`)[1].split(`</div>`)[0].split(`<a href="`)[1].split(`" class`)[0]})}catch(e){console.log(e);res.send(JSON.stringify({creator: "Rafasurya",status: false,msg: "terjadi kesalahan"}, null, 4));return}
	dt.endsWith("jpg") ? res.header("Content-Type",'image/jpg') : res.header("Content-Type",'image/png')
buff = await getBuff(`${dt.trim()}`)
res.end(buff,"binary")
});
...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Apicca') return res.json(loghandler.invalidKey)
...
router.get('/thumbnail', async (req, res, next) => {
	       waif = (await axios.get(`https://raw.githubusercontent.com/MurphyBot/datayo/main/thumbnail.json`)).data
	       result = waif[Math.floor(Math.random() * (waif.length))]
	       data = await getBuffer(result)
           await fs.writeFileSync(__path +'/tmp/thumbnail.png', data)
           await res.sendFile(__path +'/tmp/thumbnail.png')
           await sleep(3000)
           await fs.unlinkSync(__path + '/tmp/thumbnail.png')
})
...
router.get('/islami/hadith', async (req, res) => {
...
router.get('/islami/masjid/list-provinsi', async (req, res) =>
...
router.get('/islami/quran', async (req, res) => {
...
router.get('/islami/wirid', async (req, res) =>
...
router.get('/islami/ayatkursi', async (req, res) =>
...
router.get('/islami/doaharian', async (req, res) =>
...
router.get('/islami/bacaanshalat', async (req, res) =>
...
router.get('/islami/niatshalat', async (req, res) =>
...
router.get('/islami/asmaulhusna', async (req, res) =>
...
router.get('/islami/niatshubuh', async (req, res) =>
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Apicca')  return res.json(loghandler.invalidKey)
...
router.get('/cewe/korea', async (req, res, next) => {
	       waif = (await axios.get(`https://raw.githubusercontent.com/MurphyBot/datayo/main/korea.json`)).data
	       result = waif[Math.floor(Math.random() * (waif.length))]
	       data = await getBuffer(result)
           await fs.writeFileSync(__path +'/tmp/cewekorea.png', data)
           await res.sendFile(__path +'/tmp/cewekorea.png')
           await sleep(3000)
           await fs.unlinkSync(__path + '/tmp/cewekorea.png')
})
...
//Asupan
router.get('/asupan/asupan',async(req,res)=>{
var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://dapuhy-api.herokuapp.com/api/asupan/asupan?apikey=Apicca',{responseType: 'arraybuffer'}).then(resp=>{bufferstory = resp.data;
res.header('content-type','video/mp4');
res.end(bufferstory,'binary');})
})
                ...
                body: `text_1=${text}&login=OK`,
                    ...
                    if (!e) {
                        ...
                        $(".thumbnail").find("img").each(function() {
                            h = $(this).attr("src")
                            var result = "https://photooxy.com/"+h
                            fetch(encodeURI(`https://api.imgbb.com/1/upload?expiration=120&key=02320582194520b4367775b536873e34&image=${result}&name=${randomTextNumber}`))
                                .then(response => response.json())
                                .then(data => {
                                    var urlnya = data.data.url,
                                        delete_url = data.data.delete_url;
...
router.get('/textmaker/party', async (req, res, next) => {
        var theme = req.query.theme,
             text = req.query.text,
             text2 = req.query.text2,
             text3 = req.query.text3,
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != 'pink' && theme != 'rainbow') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://dapuhy-api.herokuapp.com/api/anime/storyanime?apikey=Apicca',{responseType: 'arraybuffer'}).then(resp=>{bufferstory = resp.data;
res.header('content-type','video/mp4');
res.end(bufferstory,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('http://fxc7-api.herokuapp.com/api/anime/neko?apikey=RdcUJgRE',{responseType: 'arraybuffer'}).then(resp=>{buffermilf = resp.data;
res.header('content-type','image/jpeg');
res.end(buffermilf,'binary');})
})

router.get('/shinobu',async(req,res)=>{
var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://justnino.herokuapp.com/api/image/shinobu',{responseType: 'arraybuffer'}).then(resp=>{buffershinobu = resp.data;
res.header('content-type','image/jpeg');
res.end(buffershinobu,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://justnino.herokuapp.com/api/image/megumin',{responseType: 'arraybuffer'}).then(resp=>{buffermegumin = resp.data;
res.header('content-type','image/jpeg');
res.end(buffermegumin,'binary');})
})

router.get('/fanart', async (req, res, next) => {
	       waif = (await axios.get(`https://raw.githubusercontent.com/MurphyBot/datayo/main/fanart.json`)).data
	       result = waif[Math.floor(Math.random() * (waif.length))]
	       data = await getBuffer(result)
           await fs.writeFileSync(__path +'/tmp/fanart.png', data)
           await res.sendFile(__path +'/tmp/fanart.png')
           await sleep(3000)
           await fs.unlinkSync(__path + '/tmp/fanart.png')
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://justnino.herokuapp.com/api/image/milf',{responseType: 'arraybuffer'}).then(resp=>{bufferwaifu = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferwaifu,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://justnino.herokuapp.com/api/image/cosplay',{responseType: 'arraybuffer'}).then(resp=>{bufferwaifu = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferwaifu,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('http://fxc7-api.herokuapp.com/api/anime/waifu?apikey=RdcUJgRE',{responseType: 'arraybuffer'}).then(resp=>{bufferwaifu = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferwaifu,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('http://fxc7-api.herokuapp.com/api/anime/husbu?apikey=RdcUJgRE',{responseType: 'arraybuffer'}).then(resp=>{bufferhusbu = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferhusbu,'binary');})
})
...
var apikeyInput = req.query.apikey
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('https://api.zeks.me/api/estetikpic?apikey=NinoWangy',{responseType: 'arraybuffer'}).then(resp=>{bufferaesthetic = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferaesthetic,'binary');})
})

router.get('/creative/welcome',async(req,res)=>{
var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
await axios.get('http://hadi-api.herokuapp.com/api/card/welcome',{responseType: 'arraybuffer'}).then(resp=>{bufferwelcome = resp.data;
res.header('content-type','image/jpeg');
res.end(bufferwelcome,'binary');})
})

router.get('/cewe/korea', async (req, res, next) => {
	       waif = (await axios.get(`https://raw.githubusercontent.com/MurphyBot/datayo/main/loli.json`)).data
	       result = waif[Math.floor(Math.random() * (waif.length))]
	       data = await getBuffer(result)
           await fs.writeFileSync(__path +'/tmp/cewekorea.png', data)
           await res.sendFile(__path +'/tmp/cewekorea.png')
...
router.get('/anime/Komikstation', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
	    search = req.query.search
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
	if(!query) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter q"})
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/manga/komikstation?query=${query}&apikey=404`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/anime/Komikfox', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
	    search = req.query.search
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
	if(!query) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter q"})
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/manga/komikfox?query=${query}&apikey=404`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/animesearch', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
	    search = req.query.search
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
	if(!search) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter search"})
       fetch(encodeURI(`https://api.lolhuman.xyz/api/anime?apikey=AurelCans&query=${search}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/anime/nhentai', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
	if (!query) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter code"})
       fetch(encodeURI(`https://justnino.herokuapp.com/api/nhentai?code=${query}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/kuis/siapaaku', async (req, res, next) => {
        var apikeyInput = req.query.apikey
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)

       fetch(encodeURI(`https://api.zeks.me/api/siapa-aku?apikey=NinoWangy`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/kuis/tebaklirik', async (req, res, next) => {
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://justnino.herokuapp.com/api/tebak/lirik`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/photooxy/pokemon', async (req, res, next) => {
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
        if (!theme) return res.json(loghandler.nottheme)
        if (theme != '1' && theme != '2' && theme != '3') return res.json(loghandler.notheme)
        if (!text) return res.json(loghandler.nottext)
...
router.get('/yutub/welcome', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
            url = req.query.url
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`http://leyscoders-api.herokuapp.com/api/welcome?name=Rafa&group=Official%20Group&profile=https://telegra.ph/file/74d047366cedf9bc64320.jpg&apikey=MIMINGANZ`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/downloader/instagram', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`https://api.zeks.me/api/ig?apikey=NinoWangy&url=${url}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/downloader/soundcloud', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`https://api.zeks.me/api/soundcloud?apikey=NinoWangy&url=${url}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/tiktok/ttdownloader', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
            url = req.query.url
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/socialmedia/ttdownloader?url=${url}&apikey=Apicca`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/ppcouple', async (req, res, next) => {
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/randomimage/couple?apikey=Apicca`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/darkjokes', async (req, res, next) => {
	...
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
       ...
       fetch(encodeURI(`https://api.zeks.me/api/darkjokes?apikey=NinoWangy`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/mediafire', async (req, res, next) => {
    ...
    if (!url) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter url"})
       ...
       fetch(encodeURI(`https://api.lolhuman.xyz/api/mediafire?apikey=AurelCans&url=${url}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/yutub/pinterest', async (req, res, next) => {
        var apikeyInput = req.query.apikey,
            url = req.query.url
            
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
        if (!query) return res.json({ status : false, creator : `${creator}`, message : "yang mau lu cari apa tak"})

       fetch(encodeURI(`https://justnino.herokuapp.com/api/pinterest?query=${query}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/ig/stalk', async (req, res, next) => {
    ...
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter username"})
       ...
       fetch(encodeURI(`https://dapuhy-api.herokuapp.com/api/socialmedia/igstalk?username=${username}&apikey=Apicca`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/tiktok/stalk', async (req, res, next) => {
    ...
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter username"})
       ...
       fetch(encodeURI(`http://leyscoders-api.herokuapp.com/api/tiktok-stalk?name=${username}&apikey=MIMINGANZ`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/github/stalk', async (req, res, next) => {
    ...
    if (!username) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter username"})
       ...
       fetch(encodeURI(`https://pencarikode.xyz/stalk/github?q=${username}&apikey=pais`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
router.get('/play', async (req, res, next) => {
	...
	if(!apikeyInput) return res.json(loghandler.notparam)
	if(apikeyInput != 'Apicca') return res.json(loghandler.invalidKey)
    if (!q) return res.json({ status : false, creator : `${creator}`, message : "kau nyari apa pantek"})

       fetch(encodeURI(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=NinoWangy&q=${q}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
...
await axios.get('https://hadi-api.herokuapp.com/api/loli',{responseType: 'arraybuffer'}).then(resp=>{bufferloli = resp.data;})
res.header('content-type','image/jpeg');
res.end(bufferloli,'binary');
})
...
router.get('/primbon/artimimpi', async (req, res, next) => {
    ...
    if(!q) return res.json({ status : false, creator : `${creator}`, message : "masukan parameter query"})
       fetch(encodeURI(`https://api.zeks.me/api/artimimpi?apikey=Apicca-key&q=${q}`))
        .then(response => response.json())
        .then(data => {
        var result = data;
             res.json({
